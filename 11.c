...
int main(int argc, char *argv[])
{
    ...
    if (close(pfd[0]) == -1)
        perror("Cann't closing Pipe Read end");
    if (close(pfd[1]) == -1)
        perror("Cann't closing Pipe Write end");
    if (wait(NULL) == -1)
        perror("Error During wait 1 for fork[1]");
    if (wait(NULL) == -1)
        perror("Error During wait 2 for fork[2]");
